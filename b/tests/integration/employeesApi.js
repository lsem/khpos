...
const mongoDbMemoryServer = require("../helpers/mongoDbMemoryServer");
...
describe("API", () => {
  ...
  describe("/employees", () => {
    ...
    it("should be CORS enabled", async () => {
      ...
      checkExpectations(await chai.request(server).get("/employees"));
      checkExpectations(await chai.request(server).post("/employees"));
      checkExpectations(await chai.request(server).patch("/employees"));
    });

    it("should return empty array on clear database", async () => {
      const res = await chai.request(server).get("/employees");
    ...
    it("Should return 400 if invalid employee is requested", async () => {
      const res = await chai.request(server).get(`/employees/invalid_staff_id`);
    ...
    it("Modification of existing employee should work fine", async () => {
      const vasylId = await app.insertEmployee({
        id: newEmployeeId(),
        firstName: "Василь",
        color: "rgb(216, 216, 216)"
      });
      ...
      const getStaffRes = await chai.request(server).get("/employees");
    ...
    it("should not allow to insert new employee if such already exist", async () => {
      ...
      await app.insertEmployee(newEmployee);
