...
class PosterAPI {
  getStorageLeftoversRequest(params) {
    let result = `${config.poster.apiLink}/storage.getStorageLeftovers?token=${config.poster.securityToken}`;
  ...
  getProductsRequest(params) {
    let result = `${config.poster.apiLink}/menu.getProducts?token=${config.poster.securityToken}`;
