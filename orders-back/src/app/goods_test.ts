...
describe("[goods]", () => {
  ...
  it("Should raise NotFound on attempt to get unexisting good", async () => {
    const storage = new InMemoryStorage();
    await expect(goods.getGoodByID(storage, AdminAsCaller, EIDFac.makeGoodID()))
  ...
  it("Should raise NotFound on attmept to deactive unexisting good", async () => {
    const storage = new InMemoryStorage();
    await expect(goods.makeGoodUnavailable(storage, AdminAsCaller, EIDFac.makeGoodID()))
        .to.be.rejectedWith(NotFoundError);
  });

  it("Should raise NotFound on attmept to remove unexisting good", async () => {
    const storage = new InMemoryStorage();
    await expect(goods.removeGood(storage, AdminAsCaller, EIDFac.makeGoodID()))
        .to.be.rejectedWith(NotFoundError);
  });
});
