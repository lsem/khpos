...
function AssignmentsTimeline(props) {
  ...
  const groupByWorkers = (jobs) => {
    ...
    _.forEach(jobs, j => {
      ...
      _.forEach(j.techMap.tasks, t => {
        let end = moment(startTime).add(t.durationMins, "minutes").valueOf()
        

        if (t.assigned) {
          _.forEach(t.assigned, a => {
            if (!assignmentsByWorkers[a.id])
              assignmentsByWorkers[a.id] = []
  ...
  const renderAssignmentTimeSpan = (assignment) => {
    ...
    return (<div style={assignmentTimeSpanStyle} className="assignmentsTimeSpan" key={assignment.key}></div>)
  }
  ...
  const renderColumns = ( assignmentsByWorkers ) => {
    ...
    return _.map(assignmentsByWorkers, (abw, key) => {
      ...
      return (
      <div className="assignmentsTimelineColumn" style={columnStyle} key={key}>
